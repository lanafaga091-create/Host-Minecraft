...
/* ============ GOOGLE OAUTH ASLI (Google Identity Services) ============
   Cara pakai:
   1. Buka https://console.cloud.google.com/apis/credentials
   2. Buat "OAuth client ID" dengan tipe "Web application"
   3. Di "Authorized JavaScript origins" tambahkan domain situs ini
      (mis. https://blockhost.com, dan http://localhost:PORT untuk tes lokal)
   4. Salin Client ID yang muncul, tempel ke variabel GOOGLE_CLIENT_ID di bawah ini
*/
...
function handleGoogleTokenResponse(tokenResponse){
  ...
  currentGoogleAccessToken = tokenResponse.access_token;
  fetch('https://www.googleapis.com/oauth2/v3/userinfo', {
    ...
    .then(profile => {
      ...
      completeLogin(user, `Berhasil masuk sebagai ${user.name}!`);
    })
    .catch(err => {
      console.error(err);
      showToast('Gagal mengambil data akun Google. Coba lagi.');
    });
}

let currentGoogleAccessToken = null;

/* Lanjutkan dengan Google — membuka popup login Google resmi (accounts.google.com) */
function openGooglePicker(){
  if(GOOGLE_CLIENT_ID.startsWith('GANTI_DENGAN')){
    showToast('GOOGLE_CLIENT_ID belum diisi di script.js. Buat OAuth Client ID di Google Cloud Console dulu.');
    return;
  }
  const client = ensureGoogleTokenClient();
  if(!client){
    showToast('Google belum siap dimuat. Tunggu sebentar lalu coba lagi.');
    return;
  }
...
function updateLoginUI(){
  ...
  if(isLoggedIn){
    ...
    iconSlot.innerHTML = currentUser.picture
      ? `<img class="user-avatar" src="${currentUser.picture}" alt="${currentUser.name}" referrerpolicy="no-referrer">`
      : `<span class="user-avatar" style="background:${currentUser.color};">${currentUser.name.charAt(0).toUpperCase()}</span>`;
...
function logoutUser(){
  if(currentGoogleAccessToken && window.google && google.accounts && google.accounts.oauth2){
    google.accounts.oauth2.revoke(currentGoogleAccessToken, ()=>{});
    currentGoogleAccessToken = null;
  }
  isLoggedIn = false;
  currentUser = null;
  saveAuthState();
...
function openProfileModal(){
  ...
  if(currentUser.picture){
    avatarBig.style.background = 'transparent';
    avatarBig.innerHTML = `<img src="${currentUser.picture}" alt="${currentUser.name}" referrerpolicy="no-referrer" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
  } else {
