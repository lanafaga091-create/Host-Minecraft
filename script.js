...
function confirmPayment(){
  ...
  setTimeout(()=>{
    status.textContent = '✅ Pembayaran terkonfirmasi!';
...
function selectTier(name){
  ...
  if(name === 'Free'){
    ...
    if(freeTrialUsedEmails.includes(currentUser.email)){
      showToast('Akun ini sudah pernah memakai jatah paket Free. Pilih paket berbayar untuk melanjutkan.');
      showPage('paket');
      return;
    }
    showToast('Paket Free dipilih! Langsung masuk ke Panel selama 30 menit, tanpa perlu bayar.');
    applyTierSpecs('Free');
    panelUnlocked = true;
    freeTrialUsedEmails.push(currentUser.email);
...
/* ============ DATABASE PEMAIN (data asli dari backend, sinkron dengan server PocketMine) ============ */
let playerDatabase = []; // diisi dari GET /api/players, bukan data dummy

async function fetchPlayerDatabase(){
  const res = await apiCall('/api/players');
  if(res && res.players){
    playerDatabase = res.players;
...
function renderPlayerDatabase(){
  ...
  if(playerDatabase.length === 0){
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;opacity:.6;">Belum ada pemain yang pernah bergabung.</td></tr>';
    return;
  }
  playerDatabase.forEach((p)=>{
    const tr = document.createElement('tr');
    const lastJoinLabel = p.lastJoin ? new Date(p.lastJoin).toLocaleDateString('id-ID', {day:'numeric', month:'short', year:'numeric'}) : '—';
    tr.innerHTML = `
      <td>${p.name}</td>
      <td class="${p.online ? 'db-status-ok' : ''}">${p.online ? 'ONLINE' : 'offline'}</td>
      <td>
        <select ${p.online ? '' : 'disabled title="Pemain harus online untuk diubah mode-nya"'} onchange="changePlayerMode('${p.name}', this.value)">
      ...
      <td>${p.playtimeLabel}</td>
      <td>${lastJoinLabel}</td>
      <td class="${p.banned ? 'db-status-banned' : 'db-status-ok'}">${p.banned ? 'DIBANNED' : 'AKTIF'}</td>
      <td class="db-actions">
        <button class="mini-btn danger-btn" ${p.online ? '' : 'disabled title="Pemain harus online untuk di-kick"'} onclick="kickPlayer('${p.name}')">KICK</button>
        <button class="mini-btn ${p.banned ? 'active-btn' : 'danger-btn'}" onclick="toggleBanPlayer('${p.name}', ${p.banned})">${p.banned ? 'UNBAN' : 'BAN'}</button>
      </td>`;
    tbody.appendChild(tr);
  });
}

async function changePlayerMode(name, mode){
  const res = await apiCall('/api/players/gamemode', 'POST', { name, mode });
  if(res.ok) showToast(`Mode permainan ${name} diubah ke ${mode}.`);
  else showToast(res.error || 'Gagal mengubah mode.');
  fetchPlayerDatabase();
}
async function kickPlayer(name){
  const res = await apiCall('/api/players/kick', 'POST', { name });
  if(res.ok) showToast(`${name} dikeluarkan dari server.`);
  else showToast(res.error || 'Gagal kick pemain.');
  fetchPlayerDatabase();
}
async function toggleBanPlayer(name, currentlyBanned){
  const res = await apiCall(currentlyBanned ? '/api/players/unban' : '/api/players/ban', 'POST', { name });
  if(res.ok) showToast(`${name} ${currentlyBanned ? 'di-unban' : 'diban'}.`);
  else showToast(res.error || 'Gagal memproses ban/unban.');
  fetchPlayerDatabase();
}

fetchPlayerDatabase();
setInterval(fetchPlayerDatabase, 5000); // sinkron berkala dengan data asli di backend
...
// Catatan: simulasi jumlah pemain palsu (startPlayerSim) sudah dihapus.
// Jumlah pemain sekarang selalu diambil dari status.playerCount yang dikirim
// backend (server.js), yaitu data ASLI hasil parsing log PocketMine-MP —
// lihat fungsi applyState() di atas.